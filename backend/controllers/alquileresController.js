...
async function stockDisponible(productoId, cantidad, fechaEntrega, fechaRecojo, excluirAlquilerId = null) {
  ...
  let sql = `
    ...
    WHERE ai.producto_id = $1
      AND a.estado NOT IN ('cerrado','pendiente_pago')
...
async function crearAlquiler(req, res) {
  const { items, fecha_entrega, fecha_recojo, direccion_evento } = req.body;
  ...
  try {
    ...
    const alq = await client.query(
      `INSERT INTO alquileres
         (cliente_id, fecha_entrega, fecha_recojo, direccion_evento,
          subtotal, descuento_pct, descuento_monto, garantia, total)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       RETURNING *`,
      [req.usuario.id, fecha_entrega, fecha_recojo, direccion_evento,
...
async function registrarPago(req, res) {
  ...
  if (!monto || !metodo) return res.status(400).json({ error: 'Faltan datos de pago' });
  ...
  try {
    ...
    await client.query(
      ...
      [id, monto, metodo, req.usuario.id]
    );

    await client.query(
      `UPDATE alquileres SET estado='confirmado', updated_at=NOW() WHERE id=$1`, [id]
    );

    await client.query(
      `INSERT INTO alquiler_eventos (alquiler_id, estado_antes, estado_nuevo, usuario_id, metodo)
       VALUES ($1,'pendiente_pago','confirmado',$2,'manual')`,
      [id, req.usuario.id]
    );

    const updated = await client.query('SELECT * FROM alquileres WHERE id=$1', [id]);
    await client.query('COMMIT');
